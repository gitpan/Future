...
Alternatively, when using the C<and_then> method, the result can be obtained
by calling the C<get> method on it, as it is passed as the only argument to
the code block.
...
This form is usually more convenient in cases where the code block might want
to simply return the initial future.
...
the same exception, the C<or_else> block is passed the failed C<Future> itself
so it can just return that.
...
The C<fmap> and C<fmap1> functions return a Future that will eventually give
the collected results of the individual item futures, thus making them similar
to perl's C<map> operator.
